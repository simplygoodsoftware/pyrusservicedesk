...
///
/// Ожидает вызова с главного потока: мутирует глобальные lastNoteId
/// и список пользователей поддержки, итерирует reference-типы кэша
/// (`PSDHelpySync` вызывает маппер на main).
