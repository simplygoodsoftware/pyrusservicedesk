 ...
 *
 * @return a [ViewRenderer] that accepts `States` via its [ViewRenderer.render] method.
 * Every `Model` is passed through the provided `getters` and `comparators` to corresponding `consumers`.
 */
internal inline fun <Model : Any> diff(block: DiffBuilder<Model>.() -> Unit): ViewRenderer<Model> {
    val builder =
...
internal open class DiffBuilder<Model : Any> {
