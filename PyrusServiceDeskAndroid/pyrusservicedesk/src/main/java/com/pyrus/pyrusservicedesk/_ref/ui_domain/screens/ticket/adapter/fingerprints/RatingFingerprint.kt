...
internal class RatingHolder(
    ...
    private val adapter: PayloadListAdapter<CommentEntry.RatingTextValues> by lazy { PayloadListAdapter(
        RatingTextFingerprint(onEvent)
    ) }
    ...
    init {
        ...
        binding.rating1Mini.setOnClickListener { onRatingClick(1) }
        binding.rating2Mini.setOnClickListener { onRatingClick(3) }
        binding.rating3Mini.setOnClickListener { onRatingClick(5) }

        binding.like1.setOnClickListener { onRatingClick(1) }
        binding.like2.setOnClickListener { onRatingClick(5) }
...
//        }
    ...
    override fun bind(builder: PayloadActionBuilder<CommentEntry.RatingSelector>) = builder.diff {
        ...
        entry::ratingTextRvVisibility.payloadCheck {
            binding.ratingTextRv.isVisible = entry.ratingTextRvVisibility//SatisfactionDisplayType.fromInt(ratingEntry.ratingSettings?.type) == SatisfactionDisplayType.Text
        }

        entry::smileLl5Visibility.payloadCheck {
            binding.smileLl5.isVisible = entry.smileLl5Visibility//SatisfactionDisplayType.fromInt(ratingEntry.ratingSettings?.type) == SatisfactionDisplayType.Emoji && ratingEntry.ratingSettings?.size == 5
        }

        entry::smileLlVisibility.payloadCheck {
            binding.smileLl.isVisible = entry.smileLlVisibility //getSmileLlVisibility(ratingEntry)
        }

        entry::likeLlVisibility.payloadCheck {
            binding.likeLl.isVisible = entry.likeLlVisibility//SatisfactionDisplayType.fromInt(ratingEntry.ratingSettings?.type) == SatisfactionDisplayType.Like
        }

        entry::ratingText.payloadCheck {
            binding.rateUsText.text = entry.ratingText
        }

        entry::rating2MiniVisibility.payloadCheck {
            binding.rating2Mini.isVisible = entry.rating2MiniVisibility//ratingEntry.ratingSettings?.size == 3
        }

        entry::ratingTextValues.payloadCheck {
            val list = entry.ratingTextValues?.sortedByDescending  { it.rating }
