...
internal class UpdateUserUseCase(
    ...
    fun updateUser() {
        ...
        PLog.d(TAG, "ET updateUser: currentUserId=$currentUserId newUserId=$newUserId -> ${if (willRefresh) "refresh()" else "SKIP refresh (same userId, no forced sync on open)"}")
        Log.d(TAG, "ET updateUser: currentUserId=$currentUserId newUserId=$newUserId willRefresh=$willRefresh")
        if (willRefresh) {
