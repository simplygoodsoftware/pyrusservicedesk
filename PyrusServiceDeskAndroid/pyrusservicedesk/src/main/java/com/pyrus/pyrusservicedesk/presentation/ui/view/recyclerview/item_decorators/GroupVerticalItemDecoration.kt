...
internal class GroupVerticalItemDecoration(
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (currentPosition == RecyclerView.NO_POSITION || currentPosition == parent.adapter?.itemCount?.minus(1)) return

        val viewHolder = parent.getChildViewHolder(view)
        val itemViewType = viewHolder?.itemViewType ?: TYPE_ANY
        ...
        if (orientation == RecyclerView.VERTICAL) {
            ...
            if (invert) with(outRect) {
                left = if (isNextTargetView) oneSideInnerDivider else outerDivider
                right = if (isPrevTargetView) oneSideInnerDivider else outerDivider
            }
            else with(outRect) {
